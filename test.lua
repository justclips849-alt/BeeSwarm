...
local SCORE_RELOCK_THRESHOLD = 10
...
local function computeCandidateScore(baseWeight, travelDist, spawnTime, now)
    local age = math.max(now - (spawnTime or now), 0)
    local ageBoost = math.max(0, 6 - age * 2.25)
    local distancePenalty = travelDist / 5
    local score = baseWeight + ageBoost - distancePenalty
    return score
end
...
-- Monsters ESP Logic
local monsterEspCache = {}
local monsterSpawners = Workspace:FindFirstChild("MonsterSpawners")

if monsterSpawners then
    RunService.RenderStepped:Connect(function()
        pcall(function()
            if not monstersEspEnabled then
                for _, drawing in pairs(monsterEspCache) do
            ...
            for _, spawner in ipairs(monsterSpawners:GetChildren()) do
                local label = spawner:FindFirstChild("TimerLabel", true)
                local position = nil
                
                local attach = spawner:FindFirstChildWhichIsA("Attachment", true)
                if attach then 
                    position = attach.WorldPosition 
                else
                    local part = spawner:FindFirstChildWhichIsA("BasePart", true)
                    if part then position = part.Position end
                end

                if label and position then
                    if not monsterEspCache[spawner] then
                        local text = Drawing.new("Text")
                        text.Center = true
                        text.Outline = true
                        text.Font = 2 -- UI Font
                        text.Size = 15
                        monsterEspCache[spawner] = text
                    end

                    local drawing = monsterEspCache[spawner]
                    local vector, onScreen = camera:WorldToViewportPoint(position)

                    if onScreen then
                        local timeText = label.ContentText
                        local isReady = (timeText == "0:00" or timeText == "" or timeText == "1:00" or timeText == "0")

                        drawing.Text = string.format("%s\n[%s]", spawner.Name, isReady and "READY" or timeText)
                        drawing.Color = isReady and Color3.new(0, 1, 0.2) or Color3.new(1, 0.2, 0.2)
                        drawing.Position = Vector2.new(vector.X, vector.Y)
                        drawing.Visible = true
                    else
                        drawing.Visible = false
                    end
                elseif monsterEspCache[spawner] then
                    monsterEspCache[spawner].Visible = false
                end
            end

            -- Cleanup for removed spawners
            for spawner, drawing in pairs(monsterEspCache) do
                if not spawner or not spawner.Parent then
                    drawing:Remove()
                    monsterEspCache[spawner] = nil
                end
            end
        end)
    end)
...
uiObjects.monstersEspToggle = gameEspSection:CreateToggle({
    Title = "Monsters ESP",
    Default = false,
    SaveKey = "monsters_esp_enabled",
    Callback = function(v)
        monstersEspEnabled = v
    end,
})
...
RunService.Stepped:Connect(function()
    ...
    if now - lastStuckCheckTime > 1 then -- Check for stuck every 1 second
        lastStuckCheckTime = now
        if lastStuckCheckPosition and currentTargetPosition then
            if (rootPart.Position - lastStuckCheckPosition).Magnitude < 1.0 then -- if moved less than 1 stud
                stuckCounter = stuckCounter + 1
            else
                stuckCounter = 0
            end
        else
            stuckCounter = 0 -- Reset if we don't have a target
        end
        lastStuckCheckPosition = rootPart.Position

        if stuckCounter >= 3 then -- Stuck for 3 seconds
            stuckCounter = 0
            if activeToken then
                -- Penalize current token so we pick another one
                visitedTokens[activeToken] = now + 5 -- Ignore for 5 seconds
                releaseActiveToken()
            else
                -- Get a new wander spot, forcing move
                wanderTarget = chooseWanderSpot(rootPart.Position, now)
                if wanderTarget then
                    wanderExpireTime = now + 2.75
                    requestMoveTo(wanderTarget, true)
                end
            end
            lastStuckCheckPosition = nil -- Reset position to give it time to move
        end
    end
            ...
            if isInField(pos) and (pos.Y - rootPart.Position.Y) < 3 then
                ...
                if dist < MAX_TOKEN_TRACK_DISTANCE then
                    ...
                    if info then
                        ...
                        local score = computeCandidateScore(weight, travelDist, info.SpawnTime or now, now)
    ...
    table.sort(candidates, function(a, b)
        if math.abs((a.Score or 0) - (b.Score or 0)) > 0.05 then
            return (a.Score or 0) > (b.Score or 0)
        end
        if a.IsPriority ~= b.IsPriority then
            return a.IsPriority
        end
        local spawnDelta = math.abs(a.SpawnTime - b.SpawnTime)
        if spawnDelta > 0.1 then
            return a.SpawnTime > b.SpawnTime
        end
        return a.Dist < b.Dist
    end)

    local bestLinkCandidate = nil
    for _, entry in ipairs(candidates) do
        if entry.TokenName and string.find(entry.TokenName, "Token Link") then
            bestLinkCandidate = entry
            break
        end
    end

    if bestLinkCandidate and (not activeToken or not activeTokenIsLink or activeToken ~= bestLinkCandidate.Part) then
        if activeToken and activeToken ~= bestLinkCandidate.Part then
            releaseActiveToken()
        end
        lockActiveCandidate(bestLinkCandidate, true)
    end
